/* ============================================================================
   SIFCNP - Extraccion Read-Only (SQL Server)
...
DECLARE @MaxSampleObjects INT = 6;
DECLARE @EnableSampling BIT = 0;

DECLARE @ObjectPattern1 NVARCHAR(100) = N'%just%';
DECLARE @ObjectPattern2 NVARCHAR(100) = N'%bolet%';
DECLARE @ObjectPattern3 NVARCHAR(100) = N'%hist%';
DECLARE @ObjectPattern4 NVARCHAR(100) = N'%marca%';
DECLARE @ObjectPattern5 NVARCHAR(100) = N'%resol%';

DECLARE @ColumnPattern1 NVARCHAR(100) = N'%ced%';
DECLARE @ColumnPattern2 NVARCHAR(100) = N'%estado%';
DECLARE @ColumnPattern3 NVARCHAR(100) = N'%fecha%';
DECLARE @ColumnPattern4 NVARCHAR(100) = N'%tipo%';
DECLARE @ColumnPattern5 NVARCHAR(100) = N'%motivo%';
...
INNER JOIN sys.tables tParent ON tParent.object_id = fkc.parent_object_id
INNER JOIN sys.schemas schParent ON schParent.schema_id = tParent.schema_id
INNER JOIN sys.columns cParent ON cParent.object_id = fkc.parent_object_id AND cParent.column_id = fkc.parent_column_id
INNER JOIN sys.tables tRef ON tRef.object_id = fkc.referenced_object_id
INNER JOIN sys.schemas schRef ON schRef.schema_id = tRef.schema_id
INNER JOIN sys.columns cRef ON cRef.object_id = fkc.referenced_object_id AND cRef.column_id = fkc.referenced_column_id
ORDER BY schParent.name, tParent.name, fk.name;
